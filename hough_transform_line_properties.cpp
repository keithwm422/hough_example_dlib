...
int main()
{
    ...
    while(i<10){
        ...
        for(int i=0;i<SIZE_I;i++){
          y[i]=(double) (i+start_x);
          x[i]= m*y[i] + b + distribution(generator);
...
// line is in x=m*y+b form
void calculate_line_properties(dpoint p1, dpoint p2, double& m, double& b){
  cout << "calculating line properties in x=m*y+b form" << endl;
...
// slope is in x=m*y + b form (not y=m*x+b)
double calculate_slope(double x1,double y1, double x2, double y2){
  return (x2-x1)/(y2-y1);
}
